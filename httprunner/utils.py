...
def load_folder_files(folder_path, recursive=True):
    ...
    """
    #判断是否存在多个路径，如果存在，递归解析路径，返回file_list（保存的是该路径下所有yml文件的路径）
...
def filter_module(module, filter_type):
    ...
    """
    #判断传入的参数filter_type，如果是"function"，则将filter_type赋值为is_function，否则赋值为is_variable
    filter_type = is_function if filter_type == "function" else is_variable
    #循环筛选模块内属性是function的对象，将其放入字典module_functions_dict
